...
export async function registerRoutes(app: Express): Promise<Server> {
  // Create HTTP or HTTPS server depending on whether SSL certificates are provided
  let server: Server;
  ...
  if (sslCertFile && sslKeyFile && fs.existsSync(sslCertFile) && fs.existsSync(sslKeyFile)) {
    ...
    server = createHttpsServer(httpsOptions, app) as unknown as Server;
    console.log("🔒 HTTPS mode enabled - SSL certificate loaded from:", sslCertFile);
  } else {
    server = createServer(app);
    if (sslCertFile || sslKeyFile) {
      console.warn("⚠️  SSL_CERT_FILE or SSL_KEY_FILE set but cert files not found - falling back to HTTP");
    }
  }
