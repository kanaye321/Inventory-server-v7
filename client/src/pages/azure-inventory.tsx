        ...
        <TabsContent value="charts">
          <div className="space-y-6">
                          ...
                          <table className="w-full text-sm">
                            <thead>
                              <tr className="border-b bg-gray-50 dark:bg-gray-800/60">
                                ...
                                <th className="text-left px-4 py-3 font-medium text-gray-700 dark:text-gray-300">Resource Group</th>
                                <th className="text-left px-4 py-3 font-medium text-gray-700 dark:text-gray-300">Location</th>
                                <th className="text-left px-4 py-3 font-medium text-gray-700 dark:text-gray-300">Subscription</th>
                              ...
                              {[...latestRecords]
                                ...
                                .map((record: any) => {
                                  ...
                                  const statusClass = record.status === 'active'
                                    ...
                                    : record.status === 'deleted' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400'
                                  ...
                                  return (
                                    <tr key={record.id} className={`border-b ${rowClass}`}>
                                      ...
                                      <td className="px-4 py-2.5 font-medium">{record.name}</td>
                                      <td className="px-4 py-2.5 text-gray-600 dark:text-gray-400">{record.type}</td>
                                      <td className="px-4 py-2.5 text-gray-600 dark:text-gray-400">{record.resourceGroup}</td>
                                      <td className="px-4 py-2.5 text-gray-600 dark:text-gray-400">{record.location}</td>
                                      <td className="px-4 py-2.5 text-gray-600 dark:text-gray-400">{record.subscriptions}</td>
                                      <td className="px-4 py-2.5">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              <Card>
                ...
                <CardContent>
                  {(() => {
                    ...
                    resources.forEach((r: any) => { const k = r.type || 'Unknown'; counts[k] = (counts[k] || 0) + 1; });
              ...
              <Card>
                <CardHeader>
                  <CardTitle>By Resource Group</CardTitle>
                  <CardDescription>Current inventory distribution</CardDescription>
                </CardHeader>
                <CardContent>
                  {(() => {
                    const counts: Record<string, number> = {};
                    resources.forEach((r: any) => { const k = r.resourceGroup || 'Unknown'; counts[k] = (counts[k] || 0) + 1; });
                    const total = Object.values(counts).reduce((s, c) => s + c, 0);
                    const sorted = Object.entries(counts).sort(([, a], [, b]) => b - a).slice(0, 8);
              ...
              <Card>
                ...
                <CardContent>
                  {(() => {
                    ...
                    const statusColor: Record<string, string> = { active: 'bg-green-500', stopped: 'bg-yellow-500', deallocated: 'bg-orange-500', deleted: 'bg-red-500' };
                    return sorted.length > 0 ? (
                      <div className="space-y-3 mt-1">
